...
exports.MoBind = class MoBind extends Base
  ...
  link: (slice, body) ->
    while slice.length > 0 and ((slice[0] instanceof MoFilter) or (slice[0] instanceof MoBind and slice[0].expression instanceof If))
      if slice[0] instanceof MoFilter
        @filter = slice[0].addTo @filter
      else
        @filter = new MoFilter(slice[0].expression.condition).addTo @filter
  ...
  compileNode: (o) ->
    ...
    return "#{if @returns then 'return ' else ''}#{expr}.#{if !@returns then 'forEach' else if moMap then 'map' else if @reduce then 'reduce' else if @last then 'map' else 'flatMap'}(#{@conditionalClosure @reduce and !moMap, v, @next, @next.compile o, LEVEL_TOP}#{if @reduce and !moMap then ', ' + @reduceVar else ''})"
