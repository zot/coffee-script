(function() {
  ...
  exports.Rewriter = (function() {
    ...
    Rewriter.prototype.rewriteMoforBindings = function() {
      var firstLine, inMofor, mostart, was;
      mostart = 0;
      inMofor = 0;
      firstLine = false;
      was = false;
      return this.scanTokens(function(token, i, tokens) {
        var tok;
        if (inMofor && token[1] === 'in') {
          tok = ['MOFORIN', 'in', token[2]];
          tok.generated = true;
          tokens.splice(i, 1, tok);
        } else if (inMofor && firstLine && (token[0] === 'TERMINATOR' || token[0] === 'INDENT')) {
          firstLine = false;
        } else if (inMofor && firstLine && token[0] === 'CALL_START' && (tokens[i + 3] != null) && tokens[i + 1][1] === 'do' && tokens[i + 2][0] === 'CALL_END' && tokens[i + 3][0] === 'INDENT') {
          if (tokens[i - 1][0] === 'IDENTIFIER' && tokens[i - 2][0] === 'MOFOR') {
            tok = ['MONADIDENT', tokens[i - 1][1], token[2]];
            tok.generated = true;
            tokens.splice(i - 1, 4, tok);
            return 0;
          }
          if (tokens[i - 1][0] === 'MOFOR') {
          ...
          tok.generated = true;
          tokens.splice(i, 3, tok);
        } else if (inMofor && firstLine && token[1] === 'do' && tokens[i - 1][0] === 'MOFOR') {
          if (tokens[i - 1][0] === 'MOFOR') {
            tok = ['MONADIDENT', '', token[2]];
            tok.generated = true;
            tokens.splice(i, 1, tok);
        ...
        } else if (inMofor && firstLine && token[1] === 'do' && (tokens[i + 2] != null) && tokens[i + 1][0] === 'CALL_END' && tokens[i + 2][0] === 'INDENT') {
          tok = ['MONADDO', 'do', token[2]];
          tok.generated = true;
          tokens.splice(i, 2, tokens[i + 1], tok);
        } else if (inMofor && token[0] === 'INDENT') {
          inMofor++;
          if (inMofor > 2) {
            inMofor = 0;
          }
        } else if (token[0] === 'MOFOR') {
          inMofor = 1;
          firstLine = true;
          mostart = i;
        } else if (inMofor && token[0] === 'OUTDENT') {
          inMofor = 0;
        }
        return 1;
      });
