(function() {
  ...
  exports.Rewriter = (function() {
    ...
    Rewriter.prototype.findMoforDos = function() {
      var mostart, onMoforLine;
      mostart = 0;
      onMoforLine = false;
      return this.scanTokens(function(token, i, tokens) {
        var tok;
        if (onMoforLine && (token[0] === 'TERMINATOR' || token[0] === 'INDENT')) {
          onMoforLine = false;
          if (tokens[i - 1][1] === 'do') {
    ...
    Rewriter.prototype.rewriteMoforBindings = function() {
      var firstLine, inMofor, moforLevel, mostack, mostart;
      inMofor = false;
      firstLine = false;
      mostart = 0;
      moforLevel = 0;
      ...
      return this.scanTokens(function(token, i, tokens) {
        ...
        if (token[0] === 'MOFOR') {
          mostack.push({
            ...
            firstLine: firstLine,
            mostart: mostart,
            moforLevel: moforLevel
          });
          inMofor = true;
          firstLine = true;
          mostart = i;
          moforLevel = 1;
        } else if (moforLevel && token[0] === 'INDENT') {
          moforLevel++;
          inMofor = moforLevel < 4;
        } else if (moforLevel && (token[0] === 'OUTDENT' || (moforLevel === 1 && token[0] === 'TERMINATOR'))) {
          moforLevel--;
          inMofor = moforLevel < 4;
          if (moforLevel < 1) {
            _ref = mostack.pop(), inMofor = _ref.inMofor, firstLine = _ref.firstLine, mostart = _ref.mostart, moforLevel = _ref.moforLevel;
          }
        } else if (inMofor && token[1] === 'in' && i > 3 && tokens[i - 1][0] === 'IDENTIFIER' && (tokens[i - 2][0] === 'MOFOR' || tokens[i - 2][0] === 'TERMINATOR' || tokens[i - 2][0] === 'INDENT' || (tokens[i - 2][0] === ',' && tokens[i - 3][0] === 'IDENTIFIER' && tokens[i - 4][0] === 'INDENT'))) {
          tok = ['MOFORIN', 'in', token[2]];
          tok.generated = true;
          tokens.splice(i, 1, tok);
        } else if (inMofor && firstLine && (token[0] === 'TERMINATOR' || token[0] === 'INDENT')) {
          firstLine = false;
        } else if (inMofor && token[0] === 'MONADDO') {
          if (i > 1 && tokens[i - 1][0] === 'IDENTIFIER' && tokens[i - 2][0] === 'MOFOR') {
            tok = ['MONADIDENT', tokens[i - 1][1], token[2]];
            tok.generated = true;
            tokens.splice(i - 1, 2, tok);
            return 0;
          } else if (i > 0 && tokens[i - 1][0] === 'MOFOR') {
            tok = ['MONADIDENT', '', token[2]];
            tok.generated = true;
            tokens.splice(i, 1, tok);
            return 0;
          } else if (i + 2 < tokens.length && tokens[i + 1][0] === 'CALL_END' && tokens[i + 2][0] === 'INDENT') {
