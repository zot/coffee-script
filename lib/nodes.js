(function() {
  ...
  exports.Mofor = Mofor = (function() {
    ...
    function Mofor(clauses, body) {
      this.body = body ? Expressions.wrap([body]) : null;
      this.firstClause = clauses[0];
      this.firstClause.link(clauses.slice(1, clauses.length), this.body);
  ...
  Mofor.onlyExprIn = function(node) {};
  exports.MoBind = MoBind = (function() {
    ...
    MoBind.prototype.children = ['variable', 'expression'];
    MoBind.prototype.link = function(slice, body) {
      while (slice.length > 0 && ((slice[0] instanceof MoFilter) || (slice[0] instanceof MoBind && slice[0].expression instanceof If))) {
        if (slice[0] instanceof MoFilter) {
          this.filters.push(slice[0]);
        } else {
          this.filters.push(new MoFilter(slice[0].expression.condition));
    ...
    MoBind.prototype.compileNode = function(o) {
      ...
      expr = "(monad = " + (this.expression.compile(o, LEVEL_LIST)) + ")";
      if (this.filters.length === 1) {
        expr = "" + expr + ".filter(" + (utility('bind')) + "(function(" + v + ") {return " + (this.filters[0].compile(o, LEVEL_LIST)) + "}, this))";
      } else if (this.filters.length > 1) {
        expr = "" + expr + ".filter(" + (utility('bind')) + "(function(" + v + ") {return (" + (((function() {
          var _i, _len, _ref, _results;
          _ref = this.filters;
          _results = [];
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            filt = _ref[_i];
            _results.push(filt.compile(o, LEVEL_LIST));
          }
          return _results;
        }).call(this)).join(') and (')) + ")}, this))";
      }
      if (this.next === null) {
        return "" + (this.returns ? 'return ' : '') + expr;
      } else {
        code = this.next.compile(o, LEVEL_TOP);
        return "" + (this.returns ? 'return ' : '') + expr + "." + (!this.returns ? 'forEach' : this.last ? 'map' : 'flatMap') + "(" + (utility('bind')) + "(function(" + v + "){" + code + "}, this))";
      }
    };
    return MoBind;
  })();
  exports.MoFilter = MoFilter = (function() {
    ...
    MoFilter.prototype.children = ['expr'];
    MoFilter.prototype.compileNode = function(o) {
      return this.expr.compile(o, LEVEL_LIST);
    };
    return MoFilter;
  })();
  ...
  Closure = {
    wrap: function(expressions, statement, noReturn) {
      ...
      args = [];
      if ((mentionsArgs = (expressions.contains(this.literalArgs)) || expressions instanceof Mofor) || (expressions.contains(this.literalThis))) {
