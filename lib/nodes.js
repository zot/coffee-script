(function() {
  ...
  exports.Mofor = Mofor = (function() {
    ...
    function Mofor(monad, clauses, body) {
      ...
      this.returns = false;
      if (this.monad != null) {
        this.firstClause.useReduce(this.monad.variables);
    ...
    Mofor.prototype.compileNode = function(o) {
      var code, containsThis, func;
      containsThis = this.firstClause.contains(Closure.literalThis);
      code = this.firstClause.compile(merge(o, {
        indent: this.tab + TAB
      }), LEVEL_PAREN);
      if (this.monad != null) {
        func = "function(" + this.monad.variables + ", " + (o.scope.freeVariable('i')) + "){" + code + "}";
        if (containsThis) {
          func = "" + (utility('bind')) + "(" + func + ", this)";
        }
        return "return " + (this.monad.expression != null ? this.monad.expression.compile(o, LEVEL_PAREN) : this.monad.variables) + ".reduce(" + func + ")";
      }
      return code;
    };
    return Mofor;
  ...
  exports.MoBind = MoBind = (function() {
    ...
    MoBind.prototype.compileNode = function(o) {
      ...
      moReturn = this.reduce && !(this.next instanceof MoBind);
      return "" + (this.returns ? 'return ' : '') + expr + "." + (!this.returns ? 'forEach' : moReturn ? 'Return' : this.reduce ? 'reduce' : this.last ? 'map' : 'flatMap') + "(" + (this.conditionalClosure(this.reduce, v, this.next, this.next.compile(o, LEVEL_TOP))) + (moReturn ? '()' : this.reduce ? ', ' + this.reduceVar : '') + ")";
    };
