(function() {
  ...
  exports.Mofor = Mofor = (function() {
    ...
    Mofor.prototype.children = ['clauses', 'body'];
    ...
    Mofor.prototype.compileNode = function(o) {
      var body, code, lastJumps, _ref;
      body = Expressions.wrap([this.body]);
      lastJumps = (_ref = last(body.expressions)) != null ? _ref.jumps() : void 0;
      code = this.firstClause.compile(merge(o, {
        indent: this.tab + TAB
      }), LEVEL_TOP);
      return "" + (this.returns ? '' : this.tab + '(function(){') + "\n" + (this.tab + TAB) + "var monad;\n" + (this.tab + TAB) + code + "\n" + (this.returns ? '' : this.tab + '})()');
    };
    return Mofor;
  })();
  exports.MoBind = MoBind = (function() {
    __extends(MoBind, Base);
    function MoBind(variable, expression) {
      this.variable = variable;
      this.expression = expression;
      this.filters = [];
      this.lines = [];
    ...
    MoBind.prototype.compileNode = function(o) {
      var code, expr, filt;
      if (this.variable === '_') {
        this.variable = o.scope.freeVariable('_');
      }
      expr = "(monad = " + (this.expression.compile(o, LEVEL_LIST)) + ")";
      if (this.filters.length === 1) {
        expr = "" + expr + ".filter(function(" + this.variable + ") {return " + (this.filters[0].compile(o, LEVEL_LIST)) + "})";
      } else if (this.filters.length > 1) {
        expr = "" + expr + ".filter(function(" + this.variable + ") {return (" + (((function() {
          var _i, _len, _ref, _results;
          _ref = this.filters;
          _results = [];
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      if (this.next === null) {
        ...
        return "" + (this.returns ? 'return ' : '') + expr + "." + (!this.returns ? 'forEach' : this.last ? 'map' : 'flatMap') + "(function(" + this.variable + "){" + code + "})";
      }
    };
    return MoBind;
  })();
