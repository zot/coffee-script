(function() {
  var Access, Arr, Assign, Base, Call, Class, Closure, Code, Comment, Existence, Expressions, Extends, For, IDENTIFIER, IS_STRING, If, In, Index, LEVEL_ACCESS, LEVEL_COND, LEVEL_LIST, LEVEL_OP, LEVEL_PAREN, LEVEL_TOP, Literal, MoBind, MoFilter, Mofor, NEGATE, NO, Obj, Op, Param, Parens, Push, Range, Return, SIMPLENUM, Scope, Slice, Splat, Switch, TAB, THIS, TRAILING_WHITESPACE, Throw, Try, UTILITIES, Value, While, YES, compact, del, ends, extend, flatten, last, merge, multident, starts, unfoldSoak, utility, _ref;
  ...
  exports.Mofor = Mofor = (function() {
    __extends(Mofor, Base);
    function Mofor(monad, clauses, body) {
      this.monad = monad;
      this.firstClause = clauses[0];
      this.firstClause.link(clauses.slice(1, clauses.length), body ? Expressions.wrap([body]) : null);
      this.returns = false;
      if (this.monad != null) {
        this.firstClause.monad = monad;
        this.firstClause.useReduce(monad.variables);
        this.firstClause.makeReturn();
      }
    }
    Mofor.prototype.children = ['firstClause'];
    Mofor.prototype.isStatement = YES;
    Mofor.prototype.makeReturn = function() {
      this.returns = true;
      this.firstClause.makeReturn();
      return this;
    };
    Mofor.prototype.compileNode = function(o) {
      var code, containsThis, _ref;
      containsThis = this.firstClause.contains(Closure.literalThis);
      if (((_ref = this.monad) != null ? _ref.expression : void 0) != null) {
        o.scope.add(this.monad.variables[0], 'var');
        code = "" + o.indent + this.monad.variables + " = " + (this.monad.expression.compile(o, LEVEL_PAREN)) + "\n" + o.indent + (this.firstClause.compile(merge(o, {
          indent: this.tab + TAB
        }), LEVEL_TOP));
      } else {
        code = "" + o.indent + (this.firstClause.compile(merge(o, {
          indent: this.tab + TAB
        }), LEVEL_TOP));
      }
      return code;
  ...
  exports.MoBind = MoBind = (function() {
    __extends(MoBind, Base);
    function MoBind(variables, expression) {
      this.variables = variables;
      this.expression = expression;
      this.lines = [];
      this.returns = false;
    }
    MoBind.prototype.children = ['expression', 'filter', 'next'];
    MoBind.prototype.useReduce = function(rvar) {
      var _ref;
      this.reduceVar = rvar;
      this.reduce = true;
      if (this.next instanceof MoBind) {
        this.next.useReduce(rvar);
      }
      if (this.expression instanceof Mofor && ((_ref = this.expression.monad) != null ? _ref.variables : void 0) === '') {
        this.expression.monad.variables = this.reduceVar;
        return this.expression.firstClause.useReduce(this.reduceVar);
      }
    };
    MoBind.prototype.link = function(slice, body) {
      while (slice.length > 0 && ((slice[0] instanceof MoFilter) || (slice[0] instanceof MoBind && slice[0].expression instanceof If))) {
        if (slice[0] instanceof MoFilter) {
          this.filter = slice[0].addTo(this.filter);
        } else {
          this.filter = new MoFilter(slice[0].expression.condition).addTo(this.filter);
          if (!body && slice.length === 1) {
            body = slice[0].expression.body;
          }
        }
        slice = slice.slice(1, slice.length);
      }
      if (this.last = slice.length === 0) {
        return this.next = body;
      } else {
        this.next = slice[0];
        return this.next.link(slice.slice(1, slice.length), body);
      }
    };
    MoBind.prototype.last = NO;
    MoBind.prototype.makeReturn = function() {
      this.returns = true;
      if (this.next) {
        this.next.makeReturn();
      }
      return this;
    };
    MoBind.prototype.assigns = function(name) {
      return this[this.context === 'object' ? 'value' : 'variable'].assigns(name);
    };
    MoBind.prototype.compileNode = function(o) {
      var expr, moMap, v;
      if (this.variables[0] === '_') {
        this.variables[0] = o.scope.freeVariable('i');
      }
      if (this.variables[1] === '_') {
        this.variables[1] = o.scope.freeVariable('i');
      }
      v = this.variables.join(',');
      expr = "" + ((!this.returns && this.expression instanceof Return ? this.expression.expression : this.expression).compile(o, LEVEL_PAREN));
      if (this.filter) {
        expr = "" + expr + ".filter(" + (this.conditionalClosure(false, v, this.filter, this.filter.compile(o, LEVEL_PAREN))) + ")";
      }
      if (!this.next) {
        return "" + (this.returns ? 'return ' : '') + expr;
      }
      moMap = this.reduce && !(this.next instanceof MoBind);
      return "" + (this.returns ? 'return ' : '') + expr + "." + (!this.returns ? 'forEach' : moMap ? 'map' : this.reduce ? 'reduce' : this.last ? 'map' : 'flatMap') + "(" + (this.conditionalClosure(this.reduce && !moMap, v, this.next, this.next.compile(o, LEVEL_TOP))) + (this.reduce && !moMap ? ', ' + this.reduceVar : '') + ")";
    };
    MoBind.prototype.conditionalClosure = function(reduce, v, n, code) {
      var str;
      str = "function(" + (reduce ? this.reduceVar + ', ' + v : v) + "){" + code + "}";
      if (n.contains(Closure.literalThis)) {
        return "" + (utility('bind')) + "(" + str + ", this)";
      } else {
        return str;
      }
    };
  ...
  exports.MoFilter = MoFilter = (function() {
    __extends(MoFilter, Base);
    function MoFilter(expr) {
      this.expr = expr;
    }
    MoFilter.prototype.addTo = function(filter) {
      if (!filter) {
        return this;
      }
      filter.next = this.addTo(filter.next);
      return filter;
    };
    MoFilter.prototype.makeReturn = function() {};
    MoFilter.prototype.children = ['expr', 'next'];
    MoFilter.prototype.compileNode = function(o) {
      return "return " + (this.next != null ? '(' : '') + (this.compileChain(o)) + (this.next != null ? ')' : '');
    };
    MoFilter.prototype.compileChain = function(o) {
      return (this.expr.compile(o, LEVEL_LIST)) + (this.next ? ') && (' + (this.next.compileChain(o, LEVEL_LIST)) : '');
    };
    return MoFilter;
  })();
